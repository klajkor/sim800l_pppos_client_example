...
post_event_unknown:
    ...
    esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_EVENT, ESP_MODEM_EVENT_UNKNOWN, (void *)line,
...
static void esp_handle_uart_data(esp_modem_dte_t *esp_dte)
{
    ...
    if (esp_dte->parent.dce->mode != MODEM_PPP_MODE && length)
    {
        ...
        if (strchr((char *)esp_dte->buffer, '\n') == NULL)
        {
            ...
            {
                bytes = uart_read_bytes(esp_dte->uart_port, esp_dte->buffer + length, 1, pdMS_TO_TICKS(100));
                length += bytes;
                ESP_LOGV("esp-modem: debug_data", "Continuous read in non-data mode: length: %d char: %x", length,
                         esp_dte->buffer[length - 1]);
            }
            esp_dte->buffer[length] = '\0';
        }
...
static esp_err_t esp_modem_dte_send_cmd(modem_dte_t *dte, const char *command, uint32_t timeout)
{
    ...
    ESP_LOGI(MODEM_TAG, "UART bytes str:%s", command);
    uart_write_bytes(esp_dte->uart_port, command, strlen(command));
    /* Check timeout */
    MODEM_CHECK(xSemaphoreTake(esp_dte->process_sem, pdMS_TO_TICKS(timeout)) == pdTRUE, "process command timeout",
                err);
    ret = ESP_OK;
err:
 ...
 *      ESP_FAIL on error
 */
static esp_err_t esp_modem_dte_send_wait(modem_dte_t *dte, const char *data, uint32_t length, const char *prompt,
                                         uint32_t timeout)
{
...
static esp_err_t esp_modem_dte_change_mode(modem_dte_t *dte, modem_mode_t new_mode)
{
    ...
    {
    case MODEM_PPP_MODE:
        MODEM_CHECK(dce->set_working_mode(dce, new_mode) == ESP_OK, "set new working mode:%d failed", err_restore_mode,
                    new_mode);
        uart_disable_pattern_det_intr(esp_dte->uart_port);
    ...
    case MODEM_COMMAND_MODE:
        ...
        uart_disable_rx_intr(esp_dte->uart_port);
        uart_flush(esp_dte->uart_port);
        uart_enable_pattern_det_baud_intr(esp_dte->uart_port, '\n', 1, MIN_PATTERN_INTERVAL, MIN_POST_IDLE,
                                          MIN_PRE_IDLE);
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    MODEM_CHECK(uart_param_config(esp_dte->uart_port, &uart_config) == ESP_OK, "config uart parameter failed",
                err_uart_config);
    ...
    {
        res = uart_set_pin(esp_dte->uart_port, config->tx_io_num, config->rx_io_num, UART_PIN_NO_CHANGE,
                           UART_PIN_NO_CHANGE);
    }
    ...
    /* Create Event loop */
    esp_event_loop_args_t loop_args = {.queue_size = ESP_MODEM_EVENT_QUEUE_SIZE, .task_name = NULL};
    MODEM_CHECK(esp_event_loop_create(&loop_args, &esp_dte->event_loop_hdl) == ESP_OK, "create event loop failed",
                err_eloop);
    /* Create semaphore */
...
esp_err_t esp_modem_set_event_handler(modem_dte_t *dte, esp_event_handler_t handler, int32_t event_id,
                                      void *handler_args)
{
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
...
esp_err_t esp_modem_start_ppp(modem_dte_t *dte)
{
    ...
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
    /* Set PDP Context */
    ESP_LOGI(MODEM_TAG, "Set modem APN: %s", esp_modem_ppp_apn);
    MODEM_CHECK(dce->define_pdp_context(dce, 1, "IP", esp_modem_ppp_apn) == ESP_OK, "set MODEM APN failed", err);
    /* Enter PPP mode */
...
{
    esp_modem_ppp_apn = new_modem_ppp_apn;
}
