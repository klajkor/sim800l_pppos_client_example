...
static esp_err_t sim800_set_working_mode(modem_dce_t *dce, modem_mode_t mode)
{
    ...
    case MODEM_PPP_MODE:
        ...
        DCE_CHECK(dte->send_cmd(dte, "ATD*99#\r", MODEM_COMMAND_TIMEOUT_MODE_CHANGE) == ESP_OK, "send command failed",
                  err);
        if (dce->state != MODEM_STATE_SUCCESS)
        {
            ...
            dce->handle_line = sim800_handle_atd_ppp;
            DCE_CHECK(dte->send_cmd(dte, "ATO\r", MODEM_COMMAND_TIMEOUT_MODE_CHANGE) == ESP_OK, "send command failed",
                      err);
...
static esp_err_t sim800_power_down(modem_dce_t *dce)
{
    ...
    DCE_CHECK(dte->send_cmd(dte, "AT+CPOWD=1\r", MODEM_COMMAND_TIMEOUT_POWEROFF) == ESP_OK, "send command failed",
              err);
...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
    ...
    ESP_LOGI(DCE_TAG, "Module: %s", sim800_dce->parent.name);
    /* Get IMEI number */
    ESP_LOGI(DCE_TAG, "Get IMEI number");
    vTaskDelay(pdMS_TO_TICKS(1000));
    DCE_CHECK(sim800_get_imei_number(sim800_dce) == ESP_OK, "get imei failed", err_io);
    ESP_LOGI(DCE_TAG, "IMEI: %s", sim800_dce->parent.imei);
    /* Get IMSI number */
    /*
    ESP_LOGI(DCE_TAG, "Get IMSI number");
    vTaskDelay(pdMS_TO_TICKS(1000));
