...
static void modem_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id,
                                void *event_data)
{
    switch (event_id)
    {
...
#error "Unsupported AUTH Negotiation"
    ...
    config.tx_io_num = PIN_MODEM_TX;
    config.rx_io_num = PIN_MODEM_RX;
    config.rts_io_num = 0;
    config.cts_io_num = 0;
    ...
    ESP_LOGI(TAG, "Calling sim800_init(dce)");
...
#error "Unsupported DCE"
    ...
    esp_netif_ppp_set_auth(esp_netif, auth_type, CONFIG_EXAMPLE_MODEM_PPP_AUTH_USERNAME,
